...
#
# CHANGES per request:
# - ✅ Voting (A/B/Tie/Skip) now ONLY saves the choice. It does NOT change cases.
# - ✅ Users must click **Next** to move to the next case.
# - ✅ Debiasing scrubber (list numbering, “critical finding/alert/notification” lines, etc.) remains.
# - ✅ Deterministic A/B blinding and fixed sequential order preserved.
#
# How to run:
...
def ui_header():
    ...
    st.caption(
        ...
        "**Impressions (A & B)**—one human and one AI. Click once to record your preference, then click **Next** to proceed."
    )
...
def record_and_advance(cfg, dataset_name, case_index, ai_is_a, choice, findings, ref, gen, row, state):
    """Now records ONLY. Does NOT advance pointer; user must click Next."""
    if not cfg["rater_id"]:
    ...
    try:
        ...
        # NOTE: no automatic pointer increment here
        st.success(f"Saved: {choice}. Click **Next** to go to the next case.")
    except Exception as e:
...
def render_dataset_tab(cfg, dataset_name: str):
    ...
    # Navigation: Previous / Next (Next advances pointer)
