...
#
# CHANGES per request:
# - ✅ One-click voting: replaced st.form + "Save & Next" with A/B/Tie/Skip buttons that immediately save & advance.
# - ✅ Removed any vestiges of the old with/without-images two-step flow.
# - ✅ Debiasing scrubber: strip numbered list markers and "critical finding / result / alert / notification" lines,
#       escalation/communication lines, signatures, "Impression:" headers, etc., from BOTH AI & Human impressions.
# - ✅ Keep A/B blinding deterministic per (dataset, case) with stable mapping across users (no seed UI).
# - ✅ Cases are presented in fixed sequential order (row 0..end) for everyone.
#
# How to run:
#   pip install streamlit pandas streamlit-authenticator pyyaml
#   streamlit run radiology_preference_app_4tabs_secure.py
...
def ui_header():
    ...
    st.caption(
        "Evaluate **four datasets** via tabs. Each tab shows clinical **Findings** and two blinded "
        "**Impressions (A & B)**—one human and one AI. Click once to record your preference."
    )
...
def record_and_advance(cfg, dataset_name, case_index, ai_is_a, choice, findings, ref, gen, row, state):
    if not cfg["rater_id"]:
        st.error("Enter your User ID in the sidebar first.")
        return
    db_row = dict(
...
def render_dataset_tab(cfg, dataset_name: str):
    ...
    with colA:
        st.markdown("**Impression A**")
        # Display read-only to avoid accidental edits; falls back gracefully if disabled isn't supported
        st.text_area(" ", A_text, height=260, label_visibility="collapsed", key=f"{dataset_name}_A_{case_index}", disabled=True)
    with colB:
        st.markdown("**Impression B**")
        st.text_area("  ", B_text, height=260, label_visibility="collapsed", key=f"{dataset_name}_B_{case_index}", disabled=True)

    if cfg["reveal"]:
        st.info(f"Admin view: In **{dataset_name}**, AI is shown as: **{'A' if ai_is_a else 'B'}**.")

    # --- One-click voting buttons (no form, no double-click) ---
    ...
    # Navigation (optional): previous only; forward is automatic on vote
    nav1, prog, nav2 = st.columns([1,6,1])
    with nav1:
        if st.button("⬅️ Previous", disabled=(state["ptr"] == 0), key=f"prev_{dataset_name}"):
            state["ptr"] = max(0, state["ptr"] - 1)
    with prog:
        st.progress((state["ptr"] + 1) / max(1, len(order)))
    with nav2:
        # Keep Next for manual browsing if needed (does not save)
        if st.button("Next ➡️", disabled=(state["ptr"] >= len(order) - 1), key=f"next_{dataset_name}"):
            state["ptr"] = min(len(order) - 1, state["ptr"] + 1)
